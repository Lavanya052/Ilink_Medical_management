    ...
    const submitSearch = () => {
        const filtered = patients.filter((patient) =>
            patient.appointments.some(appointment =>
                appointment.id.toString().includes(appointmentId)
            )
        );

        if (filtered.length === 0) {
            setNoPatientsMessage("No patients found with the provided Appointment ID.");
        } else {
            setNoPatientsMessage(""); // Clear the message if patients are found
        }
